export default {
  ...
  knowledgeBase: {
    ...
    image: {
      uploadTitle: "이미지 업로드",
      uploadHint: "JPG/PNG/GIF/WebP/BMP 형식 지원, 파일당 최대 20MB",
      dropHint: "이미지를 여기로 드래그하거나 클릭하여 업로드",
      selectFile: "파일 선택",
      uploading: "업로드 중...",
      ...
      noImages: "이미지 없음",
      noImagesDesc: "이미지를 업로드하여 이미지 지식 베이스를 구축하세요",
      fileTooLarge: "파일 크기는 20MB를 초과할 수 없습니다",
  ...
  knowledgeEditor: {
    ...
    basic: {
      ...
      typeImage: "이미지",
      typeDescription:
        "FAQ 유형은 구조화된 Q&A 데이터에 적합합니다. 문서 유형은 파일 파싱과 청킹을 지원합니다. 이미지 라이브러리는 OCR 및 역이미지 검색을 지원합니다. Wiki 유형은 LLM을 통해 상호 연결된 지식 페이지를 자동으로 구축합니다.",
      typeImageDescription: "이미지 KB는 VLM 모델을 통해 이미지 텍스트(OCR)를 자동 인식하고 설명을 생성하여 텍스트-이미지 및 이미지-이미지 검색을 지원합니다.",
      imageUploadHint: "JPG/PNG/GIF/WebP/BMP 형식 지원, 파일당 최대 20MB",
      imageVlmRequired: "이미지 KB에는 VLM 모델과 Embedding 모델이 필요합니다",
      imageVlmNotEnabled: "이미지 KB는 VLM 모델을 활성화해야 합니다 (모델 설정에서 VLM 모델을 선택하세요)",
      imageEmbeddingRequired: "이미지 KB에는 Embedding 모델이 필요합니다",
      nameLabel: "지식베이스 이름",
    ...
    image: {
      title: "이미지 라이브러리",
      subtitle: "이미지를 업로드하여 OCR 텍스트 인식 및 콘텐츠 설명을 자동 생성, 텍스트-이미지 및 이미지-이미지 검색 지원",
    },
    faq: {
      title: "Q&A",
      subtitle:
